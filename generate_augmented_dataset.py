...
def get_eye_squares(landmarks, scale_horizontal=3.3, scale_vertical=3, min_square_size=[60,70], max_square_size=[150,100], horizontal_offset=0.1):
...
def generate_augmented_dataset(dataset_dir: Path,
    ...
    for h5_path in dataset_dir.iterdir()[:10]:
        ...
        with h5py.File(h5_path, 'r', swmr=True) as source_f:
            ...
            for index, cam_index, face_gaze, face_head_pose, face_mat_norm, face_patch, frame_index, landmarks_norm in zip(
                ...
                before_painted_image = np.array(before_painted_image)[:,:,::-1]
                ...
                # im_show = cv2.hconcat([face_patch, before_painted_face_patch, augmented_face_patch, ])
                im_show = cv2.hconcat([face_patch, augmented_face_patch, ])
...
class ImageAugmentor():
    ...
    def generate_augmentated_image(self, 
        image: Image, 
        landmarks, 
        stablediffusion_config={'prompt': "photo of a human face ", "negative_prompt": "deformed, bad anotomy, eye corner not aligned", "seed": 0, "scale": 9, "steps": 50, "strength": 0.7},
        ...
        recalculated_landmarks = None # self.lm_detection_model.get_landmarks(np.array(image))
        ...
        left_eye_square, right_eye_square = get_eye_squares(recalculated_landmarks, scale_horizontal=3.3, scale_vertical=4, min_square_size=[60,60], max_square_size=[160,90], horizontal_offset=0.1)
        ...
        if return_generated_image:
            return inpainted_image, generated_image_copy
        else:
...
if __name__ == "__main__":
    ...
    dataset_dir = '/media/jqin/disk1/Datasets/xgaze_512'
    output_dir = '/media/jqin/disk1/Datasets/xgaze_512_diffusion'
    ...
    generate_augmented_dataset(dataset_dir=Path(dataset_dir),
                               ...
                               sample_ratio=1,
                               cam_index_to_use=[0,1],
                               stablediffusion_config={'prompt': "photo of a human's face", "negative_prompt": "deformed, bad anotomy", "seed": 0, "scale": 9, "steps": 50, "strength": 1},
